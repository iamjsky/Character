...
public class MatchingContactsAdapter extends ArrayAdapter {
    ...
    public MatchingContactsAdapter(Context context, ArrayList<MUserList.User> items, OnClickListener p_listener) {
        super(context, items);
        _context = context;
...
//                btn_pick.setVisibility(View.VISIBLE);
            ...
            fly_cover.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (status != 2)
                        return;
            ...
            tvType.setText(mUser.login_type == 1 ? "페이스북" : mUser.login_type == 2 ? "카카오" : mUser.login_type == 3 ? "네이버" : "이메일");
...
//                imv_blur.setVisibility(View.VISIBLE);
//                imv_backside.setVisibility(View.GONE);
//            } else if (mUser.is_choose_state == 1) {
...
//            btn_pick.setVisibility(mUser.pay_title.equals("") ? View.VISIBLE : View.GONE);

            if (status == 0) {
                cardPlayHandler.sendEmptyMessageDelayed(0, 1700);
            }
        }
